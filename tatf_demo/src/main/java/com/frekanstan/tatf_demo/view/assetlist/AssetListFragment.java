...
import com.frekanstan.tatf_demo.databinding.AssetListFragmentBinding;
...
public class AssetListFragment extends Fragment implements ISearchableListFragment {
    ...
    private long lastViewedAssetId = 0;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        model.getLiveData((result, error) -> context.runOnUiThread(() -> context.hideProgBar()))
                .observe(getViewLifecycleOwner(), assets -> { //every time live data changes
                    ...
                    context.setFooterText(String.format(context.getLocale(), context.getString(R.string.number_assets), assets.size()));
                    if (assets.size() == 1 && listType.equals("nofilter")) {
                        val onlyAssetId = Objects.requireNonNull(assets.get(0)).getId();
                        if (lastViewedAssetId != onlyAssetId) {
                            lastViewedAssetId = onlyAssetId;
                            val input = new Bundle();
                            input.putLong("assetId", onlyAssetId);
                            context.nav.navigate(R.id.assetDetailsFragment, input);
                        }
                    }
                });
        if (init) {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (itemId == R.id.add_filter_is_labeled) {
            ...
            adapter.getSelectableList().selectAll(listType);
            return true;
        } else if (itemId == R.id.clear_selection) {
            adapter.getSelectableList().clearSelection();
            return true;
        }
