...
import com.frekanstan.dtys_mobil.app.assets.AssetRepository;
...
public class AssetListAdapter extends PagedListAdapter<Asset, AssetListAdapter.AssetListViewHolder> {
    ...
    private static final DiffUtil.ItemCallback<Asset> DIFF_CALLBACK = new DiffUtil.ItemCallback<Asset>() {
        ...
        public boolean areContentsTheSame(@NonNull Asset oldItem, @NonNull Asset newItem) {
            return  Objects.equals(oldItem.getRemoteId(), newItem.getRemoteId()) &&
                    Objects.equals(oldItem.getRegistrationCode(), newItem.getRegistrationCode()) &&
    ...
    class AssetListViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnLongClickListener {
        ...
        TextView typeDefinition, assignedPerson, brand, model, remoteId, budgetType, registrationCode;
        ...
        AssetListViewHolder(View itemView) {
            ...
            remoteId = itemView.findViewById(R.id.asset_card_remote_id);
            countingStatus = itemView.findViewById(R.id.asset_card_status_image);
            budgetType = itemView.findViewById(R.id.asset_card_budget_type);
            registrationCode = itemView.findViewById(R.id.asset_card_registration_code);
        ...
        void bindTo(Asset asset) {
            //type or biotype
            if (asset.getBiomedicalType() != null)
                typeDefinition.setText(asset.getBiomedicalType());
            else
                typeDefinition.setText(asset.getAssetTypeDefinition());

            //reg no
            setTextWithDrawable(context, registrationCode, asset.getRegistrationCode());

            //remote id
            val rid = asset.getRemoteId();
            if (rid != null)
                setTextWithDrawable(context, remoteId, rid);
            else
                setTextWithDrawable(context, remoteId, "");

            val splittedBudget = asset.getBudgetType().getDefinition().split(" ");
            setTextWithDrawable(context, budgetType, "" + splittedBudget[0].charAt(0) + splittedBudget[1].charAt(0)); //budget type
            ...
            /*var ac = getAssetDao().getAssignmentChange(asset.getId());
            ...
            if (imgFile.exists()) {
                ...
                assetImage.setImageResource(finder.findIconId(asset.getAssetType().getAssetCode()));
