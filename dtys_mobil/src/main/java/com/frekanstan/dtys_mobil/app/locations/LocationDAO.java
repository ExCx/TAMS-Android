...
import com.frekanstan.dtys_mobil.data.Location_;
    ...
    public Query<Location> createFilteredQuery(Bundle input) {
        ...
        if (!Strings.isNullOrEmpty(q)) { //word filter
            ...
            builder.startsWith(Location_.name, q).or().contains(Location_.name, " " + q).or()
                    .startsWith(Location_.name, qu).or().contains(Location_.name, " " + qu).or()
                    .startsWith(Location_.remoteId, q).or().contains(Location_.remoteId, " " + q).or()
                    .startsWith(Location_.remoteId, qu).or().contains(Location_.remoteId, " " + qu);
        }
