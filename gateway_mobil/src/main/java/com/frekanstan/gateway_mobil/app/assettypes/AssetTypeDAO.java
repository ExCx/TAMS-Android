package com.frekanstan.gateway_mobil.app.assettypes;
...
import com.frekanstan.gateway_mobil.data.AssetType_;
...
public class AssetTypeDAO extends DAO<AssetType>
{
    ...
    public long put(AssetType item) {
        item.setDepth(item.getAssetCode().split("-").length);
    ...
    public void putAll(List<AssetType> items) {
        for (var x : items)
            x.setDepth(x.getAssetCode().split("-").length);
        getBox().put(items);
    }
    ...
    public IAssetType findTypeOfDepth(IAssetType lowestType, int depth) {
        ...
        if (lowestDepth <= depth)
            ...
            val splitted = lowestType.getAssetCode().split("-");
    ...
    public Query<AssetType> createFilteredQuery(Bundle input) {
        ...
        val assetIds = input.getLongArray("assetIds");
        val remoteIds = input.getStringArray("remoteIds");
        ...
        if (countingOpId != 0) {
            ...
            if (remoteIds != null)
                builder.in(AssetType_.remoteId, remoteIds).or().in(AssetType_.id, Longs.toArray(typeIds));
            else
        ...
        if (assetIds != null) {
            var typeIds = new ArrayList<Long>();
            val assets = AssetDAO.getDao().getBox().get(assetIds);
            for (val asset : assets)
                typeIds.add(asset.getAssetTypeId());
        ...
        if (!Strings.isNullOrEmpty(assetCode))
            builder.equal(AssetType_.assetCode, assetCode).or().startsWith(AssetType_.assetCode, assetCode + "-");
