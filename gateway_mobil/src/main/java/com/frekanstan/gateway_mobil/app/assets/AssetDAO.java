...
import com.frekanstan.gateway_mobil.app.acquisition.BudgetDAO;
...
import com.frekanstan.gateway_mobil.app.settings.SettingDAO;
...
import com.frekanstan.gateway_mobil.data.AssignmentChange_;
    ...
    public Query<Asset> createFilteredQuery(Bundle input) {
        ...
        if (personId != 0) { //person filter
            if (!Strings.isNullOrEmpty(listType) && listType.equals("foreign"))
                builder.equal(Asset_.personToAssignId, personId);
            else
        ...
        if (locationId != 0) { //location filter
            if (!Strings.isNullOrEmpty(listType) && listType.equals("foreign"))
                builder.equal(Asset_.locationToAssignId, locationId);
            else
        ...
        if (!Strings.isNullOrEmpty(q)) { //word filter
            val qu = latinize(q);
            builder.startsWith(Asset_.brandNameDefinition, q).or().contains(Asset_.brandNameDefinition, " " + q).or()
                    ...
                    .contains(Asset_.registrationCode, q).or()
                    .contains(Asset_.remoteId, q).or()
                    ...
                    .equal(Asset_.rfidCode, q).or().endsWith(Asset_.rfidCode, q);
        }
